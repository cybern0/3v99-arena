## Player.gd — Controleur joueur (mobile + desktop)
...
#  Variables internes
# ─────────────────────────────────────────────
## Reference au WorldManager — DOIT etre declare avant tout usage
...
func set_world_manager(wm: Node) -> void:
	...
	if wm and wm.has_method("register_character"):
		wm.call("register_character", self)
...
func _ready() -> void:
	...
	var col := find_child("CollisionShape3D2", true, false)
	if col:
		col.add_to_group("player")

	print("[Player] Initialise — mobile: ", enable_mobile_controls)
...
func _connect_hud_signals() -> void:
	...
	for sig in conns:
		if _game_hud.has_signal(sig) and not _game_hud.is_connected(sig, conns[sig]):
			_game_hud.connect(sig, conns[sig])
...
func _play_animation(anim_name: String) -> void:
	if not _animation_player or not _animation_player.has_animation(anim_name):
		return
...
#  Handlers HUD
# ─────────────────────────────────────────────
func _on_jump_pressed()  -> void: _jump_requested = true; jump_pressed.emit()
func _on_punch_pressed() -> void: _perform_attack("punch")
func _on_kick_pressed()  -> void: _perform_attack("kick")

func _on_run_down() -> void:
	_run_held = true
	if enable_mobile_controls: _input_direction = Vector2(0.0, -1.0)

func _on_run_up() -> void:
	_run_held = false
	if enable_mobile_controls: _input_direction = Vector2.ZERO

# ─────────────────────────────────────────────
#  Input tactile (camera)
# ─────────────────────────────────────────────
...
#  API publique
# ─────────────────────────────────────────────
func do_jump()  -> void: _jump_requested = true
func set_move_input(direction: Vector2) -> void: _input_direction = direction
func stop_movement() -> void: _input_direction = Vector2.ZERO; _run_held = false
func get_is_moving() -> bool: return _is_moving
func get_input_direction() -> Vector2: return _input_direction

func set_canvas_visible(vis: bool) -> void:
	if _canvas_layer: _canvas_layer.visible = vis
func hide_mobile_controls() -> void: set_canvas_visible(false)
func show_mobile_controls() -> void: set_canvas_visible(true)

# ─────────────────────────────────────────────
#  Combat & vie
# ─────────────────────────────────────────────
func take_damage(amount: float, _attacker: Node = null) -> void:
	print("[Player] Degats reçus: ", amount)
	if _game_hud:
